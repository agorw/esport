class Calendar
{
  ...
  loadMonth(date)
  {
    ...
    for(let i=0; i<date.getDay(); i++)
    {
      ...
      cellEvent.classList.add('event');
      this.content.appendChild(cellEvent);
      cell.append(cellEvent);
    }
    ...
    for(let i=1; i<=monthLength; i++)
    {
      ...
      {
        cell.classList.replace('other', 'today');
      }
    }
  }
}
...
// PARTIE DU SCRIPT (ecoute d'evenement/animations)
// --> Ajout d'une span tel que:
  // let span = document.createElement('span');
  // span.classList.add('category', 'orange/blue/yellow/green');
  // this.content.appendChild(span);
  // cellEvent.append(span);